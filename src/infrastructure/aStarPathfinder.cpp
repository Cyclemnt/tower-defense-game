...
#include "include/infrastructure/aStarPathfinder.hpp"
...
namespace tdg::core {
    ...
    std::vector<const Tile*> AStarPathfinder::findPath(const Tile* start, const Tile* goal, bool ignoreTowers) const {
        ...
        while (!openSet.empty()) {
            ...
            for (const Tile* neighbor : m_map.neighbors(current->tile)) {
                // If the neighbor is not walkable, skip it.
                // Or, if ignoreTowers == true, accept OpenZones regardless of occupation.
                if (!(neighbor->walkable() || (ignoreTowers && neighbor->type == TileType::Open))) continue;
