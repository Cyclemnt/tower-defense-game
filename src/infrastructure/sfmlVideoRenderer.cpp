...
namespace tdg::infra {
    ...
    void SFMLVideoRenderer::drawSprite(const std::string& spriteId, float x, float y, float size, utils::Color color) {
        try {
            if (m_worldCoordinates) { x *= *m_tileSize; y *= *m_tileSize; size *= *m_tileSize; }
    ...
    void SFMLVideoRenderer::drawRectangle(float x, float y, float width, float height, utils::Color fill, float thickness, utils::Color outline) {
        if (m_worldCoordinates) { x *= *m_tileSize; y *= *m_tileSize; width *= *m_tileSize; height *= *m_tileSize; thickness *= *m_tileSize; }
    ...
    void SFMLVideoRenderer::drawLine(float x1, float y1, float x2, float y2, float width, utils::Color fill, float thickness, utils::Color outline) {
        if (m_worldCoordinates) { x1 *= *m_tileSize; y1 *= *m_tileSize; x2 *= *m_tileSize; y2 *= *m_tileSize; width *= *m_tileSize; thickness *= *m_tileSize; }
    ...
    void SFMLVideoRenderer::drawCircle(float x, float y, float radius, utils::Color fill, float thickness, utils::Color outline) {
        if (m_worldCoordinates) { x *= *m_tileSize; y *= *m_tileSize; radius *= *m_tileSize; thickness *= *m_tileSize; }
        ...
        if (thickness > 0.0f) circle.setOutlineThickness(thickness);
        circle.setOutlineColor(sf::Color(outline.r, outline.g, outline.b, outline.a));
        m_window->draw(circle);
    }

    void SFMLVideoRenderer::drawText(const std::string& text, float size, float x, float y, utils::Color color) {
        if (m_worldCoordinates) { x *= *m_tileSize; y *= *m_tileSize; }
