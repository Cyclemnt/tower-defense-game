...
namespace tdg::core {
    ...
    void Laser::update(float dt, const std::vector<CreaturePtr>& creatures) {
        ...
        while (m_target && m_cooldown <= 0.0f) {
            ...
            // visualEffects.push_back(std::make_unique<LaserEffect>(sf::Vector2f(position), m_target->getPosition()));
        }
    }
