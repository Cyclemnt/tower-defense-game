...
Pathfinder::Pathfinder(const Map& m) noexcept
    : map(m) {}

int Pathfinder::heuristic(const Tile* a, const Tile* b) const noexcept {
    // Manhattan distance (no diagonal movement).
    sf::Vector2i d = b->getPosition() - a->getPosition();
    return std::abs(d.x) + std::abs(d.y);
}

std::vector<const Tile*> Pathfinder::findPath(const Tile* start, const Tile* goal, bool ignoreTowers) const {
